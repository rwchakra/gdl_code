...
class E_GCL_mask(E_GCL):
    ...
    def forward(self, h, edge_index, coord, node_mask, edge_mask, edge_attr=None, node_attr=None, n_nodes=None):
        ...
        edge_feat = edge_feat * edge_mask # Eq (5), soft estimation

        # TO DO: edge_feat = edge_feat * edge_mask

        #coord = self.coord_model(coord, edge_index, coord_diff, edge_feat, edge_mask) #No need to update as x is static
        h, agg = self.node_model(h, edge_index, edge_feat, node_attr) #Eq (6)
    ...
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, attention=False, node_attr=1):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i,
                            GCL(self.hidden_nf+n_node_attr, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, act_fn=act_fn, recurrent=True, attention=attention))
